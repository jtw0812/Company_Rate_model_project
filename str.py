...
credit_model = load_model("./credit_model.h5", compile=False)
...
text = "딥러닝 안면인식 프로젝트로 안면인식을 통한 보안 절차를 수립하여 편리한 보안 서비스를 제공하는 사업입니다."

bow_vector = lda_model.id2word.doc2bow(get_tokens_comp(text))

# 주제 추론
topic_distribution = lda_model[bow_vector]
topic_value = pd.read_csv("./topic_value.csv")

# 가장 높은 확률을 가지는 주제 찾기
highest_prob_topic = max(topic_distribution, key=lambda item: item[1])
print(highest_prob_topic)

highest_topic_num = highest_prob_topic[0]  # highest_topic은 (주제 번호, 확률) 튜플이므로 [0]으로 주제 번호 추출

# topic 번호에 해당하는 행 찾기
topic_row = topic_value[topic_value['lda_topics'] == highest_topic_num]

# RND_value_mean 출력
rnd_value_mean = topic_row['RND_value_mean'].values[0]
print(f"기술 가치 (RND_value_mean): {rnd_value_mean}")